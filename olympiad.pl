...
%% displaySol:
displaySol(M):- write('Team: '), nl, student(S), member(chosen(S), M), expertise(S,_), write(S), nl, fail.
displaySol(_):- nl.

satVariable( chosen(S) ):-
    student(S)
.
...
atMostOneExpertise:-
    info(_,_,X),
    findall(chosen(S), (expertise(S,X), student(S)), L),
    atMost(1,L),
    fail.
atMostOneExpertise.
